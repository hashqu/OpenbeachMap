var map, featureList, plagesSearch = [], theaterSearch = [], museumSearch = [];
...
$("#full-extent-btn").click(function() {
  map.fitBounds(plages.getBounds());
  $(".navbar-collapse.in").collapse("hide");
  return false;
});
...
/* Basemap Layers */
var mapquestOSM = L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {
	maxZoom: 18,
...
$.getJSON("data/plages.geojson", function (data) {
  plages.addData(data);
});
...
$(document).one("ajaxStop", function () {
  $("#loading").hide();
  sizeLayerControl();
  /* Fit map to plages bounds */
  map.fitBounds(plages.getBounds());
  featureList = new List("features", {valueNames: ["feature-name"]});
  featureList.sort("feature-name", {order:"asc"});

  var plagesBH = new Bloodhound({
    name: "Plages",
  ...
  $("#searchbox").typeahead({
    ...
    name: "plages",
    displayKey: "name",
    source: plagesBH.ttAdapter(),
    templates: {
      header: "<h4 class='typeahead-header'>plages</h4>"
    }
  }, {
    name: "Theaters",
    displayKey: "name",
